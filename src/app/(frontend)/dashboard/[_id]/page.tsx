...
import { Mail, Plus, GraduationCap, Book, ArrowUpRight, Clock, Eye } from "lucide-react"
...
const InfoDisplay: React.FC<{ label: string; value: string | null | undefined; isMultiline?: boolean }> = ({ label, value, isMultiline = false }) => (
...
export default async function DashboardUserPage({ params }: DashboardUserPageProps) {
  ...
  const joinedAt = format(new Date(user.createdAt), 'MMM dd, yyyy')
  ...
  return (
    <ScrollRevealWrapper>
      <div className="max-w-4xl mx-auto px-6 py-10 space-y-16 pb-24">
        
        {/* 1. HEADER SECTION */}
        <header className="reveal flex flex-col md:flex-row md:items-end justify-between gap-8 pb-10 border-b border-[rgb(var(--color-border)/0.5)]">
          ...
          <div className="reveal delay-100 flex items-center gap-3">
            {isOwner ? (
              <Link href={`/posts/new`} className="px-6 py-3 rounded-xl bg-[rgb(var(--color-fg))] text-[rgb(var(--color-bg))] text-sm font-bold hover:bg-[rgb(var(--color-accent))] hover:text-[rgb(var(--color-accent-fg))] transition-all shadow-lg">
                + Create New Post
              </Link>
            ) : isAuthenticated ? (
              <>
              ...
              <Link href="/posts/new" className="px-6 py-3 rounded-xl bg-[rgb(var(--color-bg-soft))] text-[rgb(var(--color-fg-muted))] text-sm font-bold border border-dashed border-[rgb(var(--color-border))] flex items-center gap-2">
                <Plus className="w-4 h-4" /> Sign in to connect
              </Link>
            )}
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12">
          ...
          <aside className="reveal delay-200 md:col-span-4 space-y-10">
          ...
          <div className="reveal delay-300 md:col-span-8 space-y-10">
